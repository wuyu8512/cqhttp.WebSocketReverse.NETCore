...
namespace cqhttp.WebSocketReverse.NETCore
{
    public static class CqHttpApi
    {
        ...
        /// <returns></returns>
        public static async Task<bool> Like(this Source source, byte times = 1)
        {
            if (JsonDocument.Parse(source.Context).RootElement.TryGetProperty("sender", out JsonElement je_sender))
            {
                if (je_sender.TryGetProperty("user_id", out JsonElement je_userId))
                {
                    if (je_userId.ValueKind == JsonValueKind.Number)
                    {
                        if (je_userId.TryGetInt64(out Int64 userId))
                        {
                            await SendRequestMessage(source, new CqHttpRequest()
                            {
        ...
        {
            string echo = $"~{message.Task}@{Guid.NewGuid().ToString()}";
            ActionResource.CallBackResults.TryAdd(echo, new TaskCompletionSource<ResponseResource>());
            var api = source.ConnectionData.RoleAndConnections.TryGetValue("API", out Connection conn);
            if (api)
            {
                message.Echo = echo;
        ...
        {
            var task = ActionResource.CallBackResults[echo].Task;
            if (await Task.WhenAny(task, Task.Delay(timeout)) == task)
            {
                ActionResource.CallBackResults.Remove(echo, out TaskCompletionSource<ResponseResource> _);
                return await task;
            }
            return null;
        }
        ...
        {
            if (ActionResource.CallBackResults.TryGetValue(echo, out TaskCompletionSource<ResponseResource> r))
            {
                r.SetResult(result);
            }
        }
