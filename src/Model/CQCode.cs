...
namespace cqhttp.WebSocketReverse.NETCore.Model
{
    ...
    public class CQCode
    {
        ...
        private CQCode(string str)
        {
            ...
            {
                this._items.Add(item.Groups[1].Value, CQDeCode(item.Groups[2].Value));
            }
            #endregion
        }
		...
		/// <returns>返回 <see cref="CQCode"/> 对象</returns>
		public static CQCode CQCode_Face(int faceId)
		{
			return new CQCode(
				CQFunction.Face,
				new KeyValuePair<string, string>("id", Convert.ToString(faceId)));
		}
		/// <summary>
		/// 获取酷Q "戳一戳" 代码
		/// </summary>
		...
		/// <returns>返回 <see cref="CQCode"/> 对象</returns>
		public static CQCode CQCode_Music(long id, string type = "qq", int style = 2)
		{
			return new CQCode(
				CQFunction.Music,
				new KeyValuePair<string, string>("id", Convert.ToString(id)),
